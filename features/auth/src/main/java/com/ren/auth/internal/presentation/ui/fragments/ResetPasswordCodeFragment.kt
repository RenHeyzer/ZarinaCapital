...
import com.ren.auth.databinding.FragmentResetPasswordCodeBinding
import com.ren.auth.internal.presentation.ui.viewmodels.ResetPasswordCodeViewModel
...
@AndroidEntryPoint
class ResetPasswordCodeFragment :
    BaseFragment<FragmentResetPasswordCodeBinding, ResetPasswordCodeViewModel>(R.layout.fragment_reset_password_code) {
    override val binding by viewBinding(FragmentResetPasswordCodeBinding::bind)
    override val viewModel by viewModels<ResetPasswordCodeViewModel>()
    ...
    private fun subscribeToResult() = with(binding) {
        viewModel.resultState.subscribeAsState(
            ...
            onSuccess = {
                findNavController()
                    .navigate(
                        ResetPasswordCodeFragmentDirections.actionResetPasswordCodeFragmentToResetPasswordFragment(
                            it.data
                        )
                    )
            }
